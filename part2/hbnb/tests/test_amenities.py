...
def test_create_amenity_as_admin(mock_facade, client, admin_token):
    ...
    mock_amenity.description = "Wireless Internet"
    ...
    assert data['description'] == "Wireless Internet"
...
def test_get_all_amenities(mock_facade, client):
    ...
    data = response.get_json()
    assert len(data) == 1
    assert data[0]['name'] == "WiFi"
    assert data[0]['description'] == "Wireless Internet"
...
def test_update_amenity_as_admin(mock_facade, client, admin_token):
    ...
    mock_updated.name = "Updated WiFi"
